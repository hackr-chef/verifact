...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Import the API key check function dynamically to avoid build issues
    const { checkRequiredApiKeys } = await import("@/lib/api-key-check");

    // Check if we have the required API keys
    const apiKeyCheck = checkRequiredApiKeys({ requireAll: false });
    console.log("API key check result:", apiKeyCheck);

    // Specifically check for Serper API key
    if (!apiKeyCheck.validKeys.includes('SERPER_API_KEY')) {
      console.warn("Serper API key is missing or invalid, returning mock result");
      return new Response(
        JSON.stringify({
          ...mockFactCheckResult,
          summary: "This is a demo result because the Serper API key is missing or invalid. Please add a valid SERPER_API_KEY to your environment variables.",
          apiKeyStatus: apiKeyCheck
        }),
        {
          status: 200,
          headers: {
